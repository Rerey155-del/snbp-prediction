...
export default function AdminPage() {
    ...
    async function handleClearData(type) {
        const typeLabel = type === 'students' ? 'seluruh data siswa' : 'seluruh data training';
        if (!confirm(`HATI-HATI! Apakah Anda yakin ingin menghapus ${typeLabel}? Tindakan ini tidak dapat dibatalkan.`)) {
            return;
        }
    ...
    const handleSavePDF = () => {
        ...
        autoTable(doc, {
            startY: 45,
            head: [['No', 'Nama', 'Kelas', 'Jurusan', 'PTN', 'Rata-rata', 'Hasil Prediksi']],
            body: tableData,
            theme: 'grid',
            headStyles: {
                fillColor: [79, 70, 229],
                ...
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">SMAN 1 Ampek Angkek - SNBP Predictor</p>
                        </div>
                    </div>
                    <button
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-gray-800">Detail Perhitungan Matematis</h3>
                                    <p className="text-sm text-gray-500">Siswa: <span className="font-semibold text-indigo-600">{activeStudent.nama}</span> | Nilai (x) = {activeStudent.rata_rata}</p>
                                </div>
                            </div>
                            <button onClick={() => setShowBreakdown(false)} className="text-gray-400 hover:text-gray-600 p-2">
                        ...
                        <div className="p-8 space-y-8">
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {Object.entries(getStatsForMath()).map(([key, s]) => (
                                        <div key={key} className={`p-4 rounded-xl border ${key === 'LULUS' ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
                                            <p className="font-bold text-gray-700 mb-2 truncate">KELAS: {key === 'TIDAK LULUS' ? 'BELUM LULUS' : key}</p>
                                            <div className="space-y-1 text-sm font-mono">
                                                <p className="flex justify-between"><span>Mean (μ<sub>c</sub>)</span> <span>{s.mean.toFixed(4)}</span></p>
                                                <p className="flex justify-between"><span>Varians (σ<sup>2</sup><sub>c</sub>)</span> <span>{s.variance.toFixed(4)}</span></p>
                                                <p className="flex justify-between"><span>Sta. Deviasi (σ)</span> <span>{Math.sqrt(s.variance).toFixed(4)}</span></p>
                                                <p className="flex justify-between"><span>Prior P(C)</span> <span>0.5000</span></p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <section className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                                ...
                                <p className="text-xs text-gray-500 italic text-center leading-relaxed px-4">
                                    Rumus ini digunakan untuk menghitung probabilitas nilai (x) masuk ke kelas tertentu berdasarkan sebaran data normal.
                                </p>
                            </section>
                            ...
                            <section className="space-y-6">
                                ...
                                {Object.entries(getStatsForMath()).map(([key, s]) => {
                                    ...
                                    const likelihood = (1 / Math.sqrt(2 * Math.PI * s.variance)) * Math.exp(exponent);
                                    const posterior = 0.5 * likelihood;

                                    return (
                                        <div key={key} className="space-y-3">
                                            <p className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2">
                                                <span className={`w-2 h-2 rounded-full ${key === 'LULUS' ? 'bg-green-500' : 'bg-red-500'}`}></span>
                                                Probabilitas Kelas {key === 'TIDAK LULUS' ? 'BELUM LULUS' : key}
                                            </p>
                                            <div className="bg-white border rounded-xl p-5 space-y-4 shadow-sm">
                                                <div className="flex flex-col gap-3">
                                                    <div className="flex items-center flex-wrap gap-2 text-sm font-mono bg-gray-50 p-3 rounded-lg border border-dashed text-gray-600">
                                                        <span>Gaussian =</span>
                                                        <span className="opacity-60">(1 / √2π({s.variance.toFixed(2)}))</span>
                                                        <span>×</span >
                                                        <span className="opacity-60">e<sup>-({activeStudent.rata_rata}-{s.mean.toFixed(2)})²/2({s.variance.toFixed(2)})</sup></span>
                                                    </div>
                                                    <div className="flex justify-between items-center px-2">
                                                        <span className="text-sm text-gray-600">P(X|C) =</span>
                                                        <span className="font-bold text-gray-800">{likelihood.toFixed(8)}</span>
                                                    </div>
                                                    <div className="flex justify-between items-center px-2 border-t pt-2 mt-2">
                                                        <span className="text-sm font-bold text-indigo-600">P(C) × P(X|C) =</span>
                                                        <span className="text-lg font-black text-indigo-700">{(posterior).toFixed(8)}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            {/* Final Decision */}
                            <div className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-3 text-center shadow-lg ${activeStudent.hasil === 'LULUS' ? 'bg-green-600/5 border-green-500' : 'bg-red-600/5 border-red-500'}`}>
                                <p className="text-sm font-bold opacity-60 uppercase tracking-widest">Keputusan Akhir</p>
                                <p className="text-sm text-gray-700 leading-relaxed max-w-md">
                                    Karena <strong>{activeStudent.hasil === 'LULUS' ? 'LULUS' : 'BELUM LULUS'}</strong> memiliki nilai probabilitas (Posterior) yang lebih tinggi, maka sistem memprediksi siswa ini:
                                </p>
                                <div className={`px-8 py-3 rounded-full font-black text-xl shadow-md ${activeStudent.hasil === 'LULUS' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
                                    {activeStudent.hasil === 'LULUS' ? 'BERPELUANG DITERIMA' : 'KURANG BERPELUANG'}
                                </div>
                            </div>
                        </div>
            ...
            <main className="max-w-7xl mx-auto px-6 py-8">
                ...
                <div className="grid md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-gray-600 font-medium">Berpeluang Diterima (LULUS)</p>
                                <p className="text-3xl font-bold text-gray-800 mt-1">
                                    {loadingStudents ? '...' : studentsData.filter(d => d.hasil === 'LULUS').length}
                                </p>
                            </div>
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-gray-600 font-medium">Kurang Berpeluang (BELUM LULUS)</p>
                                <p className="text-3xl font-bold text-gray-800 mt-1">
                                    {loadingStudents ? '...' : studentsData.filter(d => d.hasil === 'TIDAK LULUS').length}
                                </p>
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="p-6 bg-blue-50 rounded-xl border-2 border-dashed border-blue-200 text-center">
                                ...
                                >
                                    ...
                                    <span className="text-gray-700 font-medium">
                                        {selectedTestFile ? selectedTestFile.name : 'Klik untuk pilih file CSV Data Uji'}
                                    </span>
                            ...
                            <div className="mt-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                                <p className="text-xs text-yellow-700 flex gap-2">
                                    ...
                                    Cukup pilih file CSV data uji, sistem akan otomatis melakukan prediksi Naive Bayes
                                    pada seluruh baris data dan menyimpannya ke database.
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-2xl shadow-lg p-8 text-white">
                            ...
                            <ol className="space-y-3 text-sm opacity-90 leading-relaxed">
                                <li className="flex gap-2">
                                    ...
                                    <span>Siapkan file CSV dengan kolom Nama, NISN, Rata-rata, Jurusan, dan PTN</span>
                                </li>
                                <li className="flex gap-2">
                                    <span className="font-bold">2.</span>
                                    <span>Unggah file pada bagian &quot;Upload Data Uji Siswa&quot;</span>
                                </li>
                                <li className="flex gap-2">
                                    <span className="font-bold">3.</span>
                                    <span>Klik &quot;Unggah & Proses Prediksi&quot; untuk melihat hasil batch</span>
                                </li>
                            </ol>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-8">
                            ...
                            <div className="space-y-3 text-sm text-gray-700">
                                <p className="leading-relaxed">
                                    Sistem ini menggunakan algoritma <strong>Gaussian Naive Bayes</strong> untuk memprediksi peluang siswa diterima melalui jalur SNBP.
                                </p>
                                <div className="p-3 bg-blue-50 rounded-lg">
                                    <p className="text-xs text-gray-600">
                                        <strong>Catatan:</strong> Prediksi berdasarkan data historis dan bersifat estimasi. Hasil aktual dapat berbeda tergantung berbagai faktor lainnya.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="mt-8 bg-white rounded-2xl shadow-lg p-8">
                    ...
                    <div className="mb-8 p-6 bg-indigo-50 rounded-xl border border-indigo-100">
                        ...
                        <p className="mt-3 text-xs text-indigo-600">
                            * Format CSV: <strong>nama, rata_rata, hasil</strong>. Mengunggah file baru akan menggantikan data lama.
                        </p>
                    </div>
                        ...
                        <>
                            <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 bg-white rounded-xl border border-gray-200 shadow-sm">
                                <div className="flex gap-2 items-center">
                                    ...
                                    >
                                        LULUS ({trainingData.filter(d => d.hasil === 'LULUS').length})
                                    </button>
                                    <button
                                    ...
                                    >
                                        BELUM LULUS ({trainingData.filter(d => d.hasil === 'TIDAK LULUS').length})
                                    </button>
                                </div>

                                <p className="text-xs text-gray-500 italic">
                                    * Menampilkan {trainingData.filter(d => trainingFilter === 'ALL' || d.hasil === trainingFilter).length} data
                                </p>
                            </div>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b-2 border-gray-300 bg-gray-50">
                                            <th className="text-left py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">No</th>
                                            <th className="text-left py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Nama</th>
                                            <th className="text-center py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Rata-rata Nilai</th>
                                            <th className="text-center py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Hasil</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {trainingData
                                            .filter(d => trainingFilter === 'ALL' || d.hasil === trainingFilter)
                                            .map((item, index) => (
                                                <tr key={index} className="border-b border-gray-100 hover:bg-indigo-50/30 transition-colors">
                                                    ...
                                                    <td className="py-4 px-6 text-sm font-semibold text-gray-800">{item.nama}</td>
                                                    ...
                                                    <td className="py-4 px-6 text-center">
                                                        <span className={`inline-block px-4 py-1 rounded-lg text-xs font-bold shadow-sm ${item.hasil === 'LULUS'
                            ...
                            <div className="mt-4 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border border-indigo-200">
                                <p className="text-xs text-gray-700">
                                    <strong>ℹ️ Informasi:</strong> Data di atas adalah data historis yang digunakan sebagai referensi
                                    dalam algoritma Gaussian Naive Bayes untuk memprediksi peluang siswa diterima SNBP.
                                    Semakin banyak data training, semakin akurat prediksi yang dihasilkan.
                                </p>
                            </div>
                        </>
                    )}
                ...
                <div className="mt-8 bg-white rounded-2xl shadow-lg p-8 border-t-4 border-green-500">
                    ...
                    {loadingStudents ? (
                        ...
                        <div className="text-center py-12 bg-gray-50 rounded-lg">
                            ...
                            <p className="text-gray-600 font-medium">Belum ada data siswa tersimpan</p>
                            <p className="text-sm text-gray-500 mt-1">Lakukan prediksi dan simpan data untuk menampilkan di sini</p>
                        </div>
                    ) : (
                        <>
                            <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 bg-white rounded-xl border border-gray-200 shadow-sm">
                                <div className="flex gap-2 items-center">
                                    <span className="text-sm font-semibold text-gray-600 mr-2">Filter Hasil:</span>
                                    <button
                                    ...
                                    >
                                        LULUS ({studentsData.filter(d => d.hasil === 'LULUS').length})
                                    </button>
                                    <button
                                    ...
                                    >
                                        BELUM LULUS ({studentsData.filter(d => d.hasil === 'TIDAK LULUS').length})
                                    </button>
                                </div>

                                <div className="flex items-center gap-3">
                                    <p className="text-xs text-gray-500 italic mr-2">
                                        * Menampilkan {studentsData.filter(d => testFilter === 'ALL' || d.hasil === testFilter).length} siswa
                                    </p>
                                    <button
                                        onClick={() => {
                                            // Header CSV
                                            let csvContent = "data:text/csv;charset=utf-8,";
                                            csvContent += "Nama,NISN,Jurusan,PTN,Rata-Rata,Hasil Prediksi\n";
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b-2 border-gray-300 bg-gray-50">
                                            <th className="text-left py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">No</th>
                                            <th className="text-left py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Nama</th>
                                            <th className="text-center py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Kelas</th>
                                            <th className="text-left py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Jurusan</th>
                                            <th className="text-left py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">PTN</th>
                                            <th className="text-center py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Rata-rata</th>
                                            <th className="text-center py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Hasil Prediksi</th>
                                            <th className="text-center py-4 px-6 text-sm font-bold text-gray-700 uppercase tracking-wider">Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {studentsData
                                            ...
                                            .map((student, index) => (
                                                <tr key={index} className="border-b border-gray-100 hover:bg-green-50/30 transition-colors">
                                                    <td className="py-4 px-6 text-sm text-gray-600">{index + 1}</td>
                                                    <td className="py-4 px-6 text-sm font-semibold text-gray-800">{student.nama}</td>
                                                    <td className="py-4 px-6 text-center text-sm text-blue-600 font-bold">{student.kelas || '-'}</td>
                                                    <td className="py-4 px-6 text-left text-sm text-gray-600">{student.jurusan || '-'}</td>
                                                    <td className="py-4 px-6 text-left text-sm text-gray-600">{student.ptn || '-'}</td>
                                                    <td className="py-4 px-6 text-center">
                                                        <span className="inline-block px-4 py-1 bg-blue-50 text-blue-700 rounded-lg font-bold border border-blue-100">
                                                            {student.rata_rata}
                                                        </span>
                                                    ...
                                                    <td className="py-4 px-6 text-center">
                                                        <span className={`inline-block px-4 py-1 rounded-lg text-xs font-bold shadow-sm ${student.hasil === 'LULUS'
                                                            ? 'bg-green-100 text-green-700 border border-green-200'
                                                            : 'bg-red-100 text-red-700 border border-red-200'
                                                            }`}>
